...
class MassiveParallelDatasetProcessor:
    ...
    def __init__(self, input_dir: str, output_dir: str = "consciencia_completa", adaptive_config: Dict = None):
        ...
        self.content_manager = SimplifiedContentManager()
            ...
            try:
                ...
                for article in article_batch:
                    ...
                    try:
                        ...
                        if articles_this_worker < 3:  # Solo log para los primeros 3 artículos
                            self.logger.log(f"🔍 DEBUG Worker-{worker_id}: Article '{article.get('title', 'N/A')[:50]}...'", force=True)
                            self.logger.log(f"🔍 DEBUG Worker-{worker_id}: Result type: {type(result)}", force=True)
                            if result:
                                self.logger.log(f"🔍 DEBUG Worker-{worker_id}: Category: {result.get('category', 'N/A')}", force=True)
                                self.logger.log(f"🔍 DEBUG Worker-{worker_id}: Conversations: {len(result.get('conversations', []))}", force=True)
                            else:
                                self.logger.log(f"🔍 DEBUG Worker-{worker_id}: Result is None/False", force=True)
                        
                    except Exception as e:
                        self.logger.log(f"❌ ERROR Worker-{worker_id} processing article '{article.get('title', 'N/A')[:50]}...': {e}", force=True)
                        result = None
                    ...
                    if result:
                        ...
                        # DEBUG: Verificar estructura de conversations
                        if articles_this_worker < 3:
                            conversations = result.get('conversations', [])
                            self.logger.log(f"🔍 DEBUG Worker-{worker_id}: Enhanced article conversations: {len(conversations)}", force=True)
                            if conversations and len(conversations) > 0:
                                sample_conv = conversations[0]
                                self.logger.log(f"🔍 DEBUG Worker-{worker_id}: Sample conversation keys: {list(sample_conv.keys())}", force=True)
